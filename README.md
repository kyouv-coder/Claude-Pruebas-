...
- **Cobertura de tests**: hay una suite (`npm test`, 47 tests) sobre la lógica más crítica (CSV injection, slugs, horarios, doble reserva, canje de giftcards y venta de productos sin sobregiro de saldo/stock, doble cobro de un turno, atomicidad al vender una giftcard, validación y aislamiento multi-tenant de comprobantes y fotos adjuntas, limpieza de rate limiting, HTML injection en el email diario), pero no cubre todo el código.
