...
- **Cobertura de tests**: hay una suite (`npm test`, 59 tests) sobre la lógica más crítica (firma y verificación de la sesión, CSV injection, slugs —incluida la carrera de slugs duplicados en el registro—, horarios, doble reserva concurrente, canje de giftcards y venta de productos sin sobregiro de saldo/stock, doble cobro de un turno, atomicidad al vender una giftcard, validación y aislamiento multi-tenant de comprobantes y fotos adjuntas, limpieza de rate limiting, HTML injection en el email diario, motor de recomendaciones del dashboard), pero no cubre todo el código.
