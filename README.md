...
- **Cobertura de tests**: hay una suite (`npm test`, 42 tests) sobre la lógica más crítica (CSV injection, slugs, horarios, doble reserva, canje de giftcards y venta de productos sin sobregiro de saldo/stock, doble cobro de un turno, validación y aislamiento multi-tenant de comprobantes y fotos adjuntas, limpieza de rate limiting), pero no cubre todo el código.
